...
def detail(request,oid):
    order = Orders.objects.filter(id=oid)
    detail = Detail.objects.filter(orderid=oid)
    context = {"detaillist":detail,"orderslist":order}
    return render(request,"myadmin/detail/index.html",context)
def orderl(request):
    list = Orders.objects.filter()
    context = {"orderslist":list}
    return render(request,"myadmin/detail/orders.html",context)
def orderldel(request,oid):
    order = Orders.objects.filter(id=oid)
    for x in order:
        x.status=3
        x.save()
    return redirect(reverse('myadmin_orderl'))
def orderlsfh(request,oid):
    order = Orders.objects.filter(id=oid)
    for x in order:
        x.status=1
        x.save()
